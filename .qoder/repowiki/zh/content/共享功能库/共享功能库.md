...
- drw/ 工程图相关功能（工程图转 DWG/DXF/PNG、可见边线提取等）
...
CheckDoc --> GetSheet["获取当前图纸"]
GetSheet --> GetViews["获取视图集合"]
GetViews --> LoopViews{"遍历视图"}
LoopViews --> |是| GetFaces["获取可见面并统计面积与类型"]
GetFaces --> GetEdges["获取可见边并识别曲线类型"]
GetEdges --> LoopViews
LoopViews --> |否| End(["函数结束"])
```
